...
class MyTheresaScraper:
    ...
    def parse(self, selector):
        ...
        for url in product_urls:
            ...
            headers = {'User-Agent': self.user_agent}
            response = requests.get("https://www.mytheresa.com"+url, headers=headers)
        
            if response.status_code == 200:
                product_selector = Selector(text=response.text)
                self.parse_product(product_selector)

            if self.counterlink <1000:

                with open("product_links_mytheresa.txt", "a") as file:
    ...
    def save_to_json(self, data):
        filename = "output_mytheresa.json"
