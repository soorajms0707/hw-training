    ...
    def clean_data(self, data):
        ...
        for key, value in data.items():
            if isinstance(value, list):
                cleaned_value = [re.sub(unicode_pattern, '', re.sub(br_pattern, '', item)).strip() if isinstance(item, str) else item for item in value]
                if key == "Breadcrumb":
                    cleaned_data[key] = " > ".join([part.strip() for part in cleaned_value])
                else:
                    cleaned_data[key] = cleaned_value
            elif isinstance(value, str):
                cleaned_value = re.sub(unicode_pattern, '', re.sub(br_pattern, '', value)).strip()
                if key == "Breadcrumb":
